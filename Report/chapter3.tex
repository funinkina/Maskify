...
\caption{System Architecture: RE-DACT Pipeline from File Ingestion to Output Delivery}
...
\caption{RE-DACT Upload Interface}
\label{fig:upload_interface}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{results_screen.png}
\caption{RE-DACT Results Interface}
\label{fig:results_interface}
\end{figure}
...
The deduplicated detection list is then passed to the masking module, which computes character-level masking ranges based on the selected redaction level. After masking is applied to each page image, the system assembles the output.

\begin{table}[htbp]
\caption{Redaction Masking Levels}
...
This approach ensures accurate redaction while preserving document structure. The modular design allows components to be tested and updated independently.

\bigskip

The data flow design prioritizes reliability and debuggability. For image inputs, the single redacted image is the final output. For PDF inputs, the redacted page images are converted back to a multi-page PDF using img2pdf, preserving the original document structure while incorporating the applied redactions.
