...
\begin{table}[htbp]
\caption{Research Gaps Addressed by RE-DACT}
\label{tab:research_gaps}
\centering
\begin{tabular}{|p{2.5cm}|p{3.5cm}|p{4cm}|}
\hline
\textbf{Research Area} & \textbf{Identified Gap} & \textbf{RE-DACT Solution} \\
\hline
System Integration & Lack of unified platform & Integrated platform with single workflow \\
\hline
Scalability & Can't handle enterprise volumes & Sub-300ms processing with in-memory results \\
\hline
Contextual Understanding & Rule-based fails on ambiguous data & Checksum validation (Verhoeff, Luhn) \\
\hline
Adaptability & One-size-fits-all approach & Three masking levels (standard, partial, full) \\
\hline
Security and Trust & No auditability & SHA-256 hash-chain audit logging \\
\hline
\end{tabular}
\end{table}
...
This foundation of understanding shapes the specific technical choices made in RE-DACT.
